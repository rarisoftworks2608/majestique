...
function ClippingCard({ item, idx, onViewClipping }) {
  ...
  return (
    ...
    >
      ...
      >
        {item.image ? (
          <Image src={item.image} alt={item.headline} className="object-contain p-2.5 transition-transform duration-600 group-hover:scale-[1.05]" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
        ) : (
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center justify-between gap-2 mb-3">
          <span
            ...
            style={{ background: `${item.pubColor}14`, color: item.pubColor, border: `1px solid ${item.pubColor}28`, whiteSpace: 'nowrap' }}
          >
            {item.publication}
          </span>
  ...
  return (
    <>
      <PageHero
        ...
        subtitle="Editorial features, award announcements, and press recognition of Majestique Landmarks, as covered by India's leading media houses."
            ...
            >
              {grouped.length === 0 ? (
                <div className="text-center py-20">
                  <Newspaper size={40} className="mx-auto mb-4" style={{ color: 'rgba(212,175,55,0.25)' }} />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      {items.map((item, idx) => (
                        <ClippingCard key={item.id} item={item} idx={idx}
                          onViewClipping={() => router.push(`/media/press-coverage/${item.slug}`)} />
                      ))}
                    </div>
                  </div>
                ))
        ...
        <div className="container-luxury text-center relative">
          <motion.div {...fU()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <Link href="/media/awards" className="btn-outline-dark">View Awards</Link>
