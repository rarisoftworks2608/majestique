...
import { AWARDS, PLATFORMS, PLATFORM_META, CATEGORY_META, CATEGORIES } from '../../data/awards'
...
function AwardCard({ award, idx, onView }) {
  ...
  const catColor = CATEGORY_META[award.category]?.color || '#D4AF37'
  const platMeta = PLATFORM_META[award.platform] || { color: '#D4AF37' }
  ...
  return (
    <motion.article
      ...
      onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 18px 56px rgba(26,26,26,0.12)'; e.currentTarget.style.transform = 'translateY(-5px)' }}
    ...
    >
      ...
      <div
        ...
        style={{ aspectRatio: '4/3', background: '#FAF6EF', flexShrink: 0 }}
        onClick={onView}
      >
        {award.image ? (
          <Image src={award.image} alt={award.award} className="object-cover transition-transform duration-700 group-hover:scale-[1.06]" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
        ) : (
          <div
            className="w-full h-full flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, #FAF6EF 0%, #FAF6EF 50%, #FAF6EF 100%)' }}
          >
            <div className="text-center">
              <div
                className="w-14 h-14 flex items-center justify-center mx-auto mb-2"
                style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold-dark))', boxShadow: '0 8px 24px rgba(212,175,55,0.35)' }}
              >
                <Trophy size={22} style={{ color: '#fff' }} />
              </div>
              <span className="font-ui text-[0.44rem] tracking-[0.22em] uppercase" style={{ color: 'rgba(212,175,55,0.6)' }}>
                {award.platform}
              </span>
            </div>
          </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center justify-between gap-2 mb-3">
          <span
            className="font-ui text-[0.46rem] tracking-[0.22em] uppercase px-2 py-0.5 overflow-hidden text-ellipsis"
            style={{ background: `${platMeta.color}14`, color: platMeta.color, border: `1px solid ${platMeta.color}28`, whiteSpace: 'nowrap' }}
          >
            {award.platform}
          </span>
          <span className="flex items-center gap-1 font-ui text-[0.62rem] tracking-widest flex-shrink-0" style={{ color: 'rgba(26,26,26,0.72)', whiteSpace: 'nowrap' }}>
  ...
  return (
    <>
        ...
        <div className="container-luxury relative">
          ...
          >
            {CATEGORIES.map(cat => {
              ...
              const catColor = CATEGORY_META[cat]?.color || '#D4AF37'
            ...
            >
              {grouped.length === 0 ? (
                <div className="text-center py-20">
                  <Trophy size={40} className="mx-auto mb-4" style={{ color: 'rgba(212,175,55,0.25)' }} />
                ...
                grouped.map(([year, items]) => (
                  <div key={year} className="mb-14">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                      ...
                      <span className="font-ui text-[0.65rem] tracking-[0.26em] uppercase" style={{ color: 'rgba(26,26,26,0.72)', whiteSpace: 'nowrap' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      {items.map((award, idx) => (
                        <AwardCard key={award.id} award={award} idx={idx}
                          onView={() => router.push(`/media/awards/${award.slug}`)} />
                      ))}
                    </div>
                  </div>
                ))
      ...
      <motion.section
        ...
        style={{ background: 'var(--cream)', borderTop: '1px solid rgba(212,175,55,0.15)' }}
      >
        <div className="container-luxury">
          <p className="font-ui text-[0.68rem] tracking-[0.36em] uppercase text-center mb-10" style={{ color: 'var(--gold-dark)' }}>
            Recognised By India&apos;s Leading Industry Platforms
          </p>
          <div className="flex flex-wrap items-start justify-center gap-6 lg:gap-12">
            {PLATFORMS.map((plat, i) => (
              ...
              >
                ...
                <span className="font-ui text-[0.6rem] tracking-[0.18em] uppercase text-center" style={{ color: 'rgba(26,26,26,0.65)', maxWidth: '90px' }}>
                  {plat.name}
                </span>
              </motion.div>
            ))}
        ...
        <div className="container-luxury text-center relative">
          <motion.div {...fU()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link href="/projects/ongoing" className="btn-gold">Explore Our Projects</Link>
              <Link href="/media/press-coverage" className="btn-outline-dark">Press Coverage</Link>
