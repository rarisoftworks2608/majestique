...
function BlogCard({ blog, index }) {
  return (
    ...
    >
      <Link href={`/media/blogs/${blog.slug}`} className="block relative overflow-hidden" style={{ aspectRatio: '16/9' }}>
        <Image src={blog.image} alt={blog.title} className="object-cover transition-transform duration-700 group-hover:scale-105" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <h3 className="font-times text-lg line-clamp-2" style={{ color: 'var(--luxury-dark)', lineHeight: '1.3', height: '2.6em', marginBottom: '0.75rem' }}>
          <Link href={`/media/blogs/${blog.slug}`} className="hover:opacity-75 transition-opacity">{blog.title}</Link>
        </h3>
        ...
        <div className="container-luxury">
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-5">
              <Link href={`/media/blogs/${featured.slug}`} className="lg:col-span-3 relative overflow-hidden block" style={{ minHeight: '320px' }}>
                <Image src={featured.image} alt={featured.title} className="object-cover transition-transform duration-700 group-hover:scale-105 absolute inset-0" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
              ...
              <div className="lg:col-span-2 p-8 lg:p-10 flex flex-col justify-center">
                ...
                <h2 className="font-times font-normal leading-snug mb-3" style={{ fontSize: 'clamp(1.4rem, 2.5vw, 2rem)', color: 'var(--luxury-dark)' }}>
                  <Link href={`/media/blogs/${featured.slug}`} className="hover:opacity-75 transition-opacity">{featured.title}</Link>
                </h2>
                ...
                <Link
                  href={`/media/blogs/${featured.slug}`}
        ...
        <div className="container-luxury text-center relative">
          <motion.div {...fU()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link href="/projects/ongoing" className="btn-gold transition-transform duration-300 hover:scale-105">Explore Our Projects</Link>
              <Link href="/contact" className="btn-outline-dark transition-transform duration-300 hover:scale-105">Get in Touch</Link>
            </div>
          </motion.div>
        </div>
      </section>
