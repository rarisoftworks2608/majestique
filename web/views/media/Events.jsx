    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2 relative overflow-hidden" style={{ minHeight: '320px', background: '#FAF6EF' }}>
          <Image src={imgSrc} alt={event.title} className="absolute inset-0 object-cover transition-transform duration-700 group-hover:scale-[1.04]" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
  ...
  if (hasDetail) {
    return (
      <motion.article {...wrapperProps}>
        <Link href={`/media/events/${event.slug}`} className="contents">{inner}</Link>
      </motion.article>
    )
  }
        ...
        <div className="container-luxury text-center relative">
          <motion.div {...fU()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link href="/contact" className="btn-gold">Register Your Interest</Link>
              <Link href="/projects/ongoing" className="btn-outline-dark">Explore Projects</Link>
            </div>
          </motion.div>
        </div>
      </section>
