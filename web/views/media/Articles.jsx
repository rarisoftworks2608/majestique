...
function ArticleCard({ article, idx, onView }) {
  const catMeta = ARTICLE_CATEGORY_META[article.category] || { color: '#D4AF37' }
  ...
  return (
    <motion.article
      ...
      onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 26px 64px rgba(26,26,26,0.16)'; e.currentTarget.style.transform = 'translateY(-6px)' }}
      onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 20px rgba(26,26,26,0.06)'; e.currentTarget.style.transform = 'translateY(0)' }}
    >
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: '3/4', background: '#FAF6EF' }}>
        <Image src={article.image} alt={article.headline} className="object-cover transition-transform duration-700 group-hover:scale-[1.06]" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
        <div className="art-shine absolute inset-0 pointer-events-none" aria-hidden />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, transparent 55%, rgba(26,26,26,0.55) 100%)' }} />
        ...
        <div className="absolute top-2.5 right-2.5">
          <span
            ...
            style={{ background: 'rgba(26,26,26,0.55)', color: '#fff', backdropFilter: 'blur(4px)' }}
          >
            {article.publication}
          </span>
        </div>
        ...
        <div className="absolute bottom-0 inset-x-0 p-5">
          <p className="flex items-center gap-1.5 font-ui text-[0.5rem] tracking-widest uppercase mb-2" style={{ color: 'rgba(250,246,239,0.7)' }}>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="pt-3 flex items-center justify-between" style={{ borderTop: '1px solid rgba(212,175,55,0.1)' }}>
...
export default function Articles() {
  ...
  return (
    <>
      <PageHero
        title="Articles"
        subtitle="The brand stories, campaigns, and festive greetings we've shared with Pune as creative features published across the city's leading publications."
      ...
      {FEATURED_ARTICLE && (
        <section className="relative overflow-hidden" style={{ background: 'var(--luxury-dark)' }}>
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 12% 15%, rgba(212,175,55,0.22) 0%, transparent 55%)' }} />
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 88% 85%, rgba(212,175,55,0.14) 0%, transparent 50%)' }} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
              ...
              >
                <div className="relative overflow-hidden" style={{ aspectRatio: '4/3', boxShadow: '0 30px 80px rgba(0,0,0,0.5)' }}>
                  <Image src={FEATURED_ARTICLE.image} alt={FEATURED_ARTICLE.headline} className="object-cover" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                  <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, transparent 45%, rgba(26,26,26,0.75) 100%)' }} />
                </div>
                <div
                  className="absolute -bottom-5 left-5 sm:left-8 flex items-center gap-2 px-4 py-2.5"
                  style={{ background: 'var(--gold)', boxShadow: '0 14px 34px rgba(212,175,55,0.5)' }}
                >
              ...
              >
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  <span
                    ...
                    style={{ background: 'rgba(212,175,55,0.16)', color: 'var(--gold)', border: '1px solid rgba(212,175,55,0.4)' }}
                  >
                    {FEATURED_ARTICLE.publication}
                  </span>
                  <span className="flex items-center gap-1.5 font-ui text-[0.6rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(250,246,239,0.45)' }}>
                ...
                <p className="font-body leading-relaxed mb-9" style={{ fontSize: '1rem', color: 'rgba(250,246,239,0.75)', maxWidth: '480px', lineHeight: 1.8 }}>
                  {FEATURED_ARTICLE.excerpt}
                </p>

                <Link
                  href={`/media/articles/${FEATURED_ARTICLE.slug}`}
                  ...
                  style={{ padding: '0.9rem 2.2rem', background: '#6B0D1A', color: '#fff' }}
                  onMouseEnter={e => { e.currentTarget.style.background = '#3D090F' }}
                  onMouseLeave={e => { e.currentTarget.style.background = '#6B0D1A' }}
                >
      ...
      <section className="section-pad relative" style={{ background: 'var(--cream)' }}>
        ...
        <div className="container-luxury relative">
          ...
          <div
            ...
            style={{ background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.16)', borderRadius: '999px', width: 'fit-content', maxWidth: '100%' }}
          >
            {ARTICLE_CATEGORIES.map(cat => {
              const CatIcon = ARTICLE_CATEGORY_META[cat]?.icon || Sparkles
              const catColor = ARTICLE_CATEGORY_META[cat]?.color || '#D4AF37'
            ...
            >
              {filtered.length === 0 ? (
                <div className="text-center py-20">
                  <Sparkles size={40} className="mx-auto mb-4" style={{ color: 'rgba(212,175,55,0.25)' }} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {filtered.map((article, idx) => (
                    <ArticleCard key={article.id} article={article} idx={idx}
                      onView={() => router.push(`/media/articles/${article.slug}`)} />
                  ))}
                </div>
              )}
            </motion.div>
      ...
      <motion.section
        ...
        style={{ background: 'white', borderTop: '1px solid rgba(212,175,55,0.15)' }}
      >
        <div className="container-luxury mb-10">
          <p className="font-ui text-[0.48rem] tracking-[0.36em] uppercase text-center" style={{ color: 'rgba(212,175,55,0.6)' }}>
            ...
            {[...ARTICLE_PUBLICATIONS, ...ARTICLE_PUBLICATIONS].map((pub, i) => (
              <div key={i} className="group flex flex-col items-center gap-2 flex-shrink-0">
                <div
                  ...
                  style={{ border: '1px solid rgba(212,175,55,0.2)', background: 'var(--cream)', boxShadow: '0 2px 12px rgba(26,26,26,0.05)', transition: 'box-shadow 0.3s, border-color 0.3s' }}
                  onMouseEnter={e => { e.currentTarget.style.boxShadow = `0 8px 28px ${pub.color}22`; e.currentTarget.style.borderColor = `${pub.color}55` }}
                  onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 2px 12px rgba(26,26,26,0.05)'; e.currentTarget.style.borderColor = 'rgba(212,175,55,0.2)' }}
                >
                  <span className="font-display font-light" style={{ fontSize: '0.92rem', color: pub.color, letterSpacing: '0.06em' }}>
                    {pub.abbr}
                  </span>
                </div>
                <span className="font-ui text-[0.44rem] tracking-[0.18em] uppercase text-center" style={{ color: 'rgba(26,26,26,0.4)', maxWidth: '90px' }}>
                  {pub.name}
                </span>
              </div>
            ))}
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: 'var(--cream)' }}>
        ...
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(212,175,55,0.05) 0%, transparent 68%)' }} />
        ...
        <div className="container-luxury text-center relative">
          <motion.div {...fU()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link href="/media/press-coverage" className="btn-gold transition-transform duration-300 hover:scale-105">Press Coverage</Link>
              <Link href="/media/awards" className="btn-outline-dark transition-transform duration-300 hover:scale-105">Awards &amp; Recognition</Link>
            </div>
          </motion.div>
        </div>
      </section>
