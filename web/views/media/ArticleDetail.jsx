'use client'

import { imgSrc } from '../../lib/imgSrc'
import Image from 'next/image'
import Link from 'next/link'
import { useRouter, useParams } from 'next/navigation'
import { useMemo } from 'react'
import { motion } from 'framer-motion'
import { ArrowLeft, Calendar, Sparkles, FileText } from 'lucide-react'
import { ARTICLES, ARTICLE_CATEGORY_META } from '../../data/articles'

export default function ArticleDetail() {
  const { slug } = useParams()
  const router = useRouter()

  const item = useMemo(() => ARTICLES.find(a => a.slug === slug), [slug])

  const related = useMemo(() => {
    if (!item) return []
    const sameCategory = ARTICLES.filter(a => a.id !== item.id && a.category === item.category)
    const rest = ARTICLES.filter(a => a.id !== item.id && a.category !== item.category)
    return [...sameCategory, ...rest].slice(0, 3)
  }, [item])

  if (!item) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--cream)' }}>
        <div className="text-center">
          <Sparkles size={48} className="mx-auto mb-5 opacity-20" style={{ color: 'var(--gold)' }} />
          <h1 className="font-times text-3xl mb-3" style={{ color: 'var(--luxury-dark)' }}>Article not found</h1>
          <p className="font-body text-sm mb-6" style={{ color: 'rgba(26,26,26,0.82)' }}>
            The article you're looking for doesn't exist or has been removed.
          </p>
          <Link href="/media/articles" className="btn-outline-gold">← Back to Articles</Link>
        </div>
      </div>
    )
  }

  const catColor = ARTICLE_CATEGORY_META[item.category]?.color || '#D4AF37'

  return (
    <>
      <div className="py-12" style={{ background: 'white' }}>
        <div className="container-luxury">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-start max-w-5xl mx-auto">

            {/* Poster image */}
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
              className="relative"
            >
              <div
                className="overflow-hidden"
                style={{ aspectRatio: '3/4', background: '#FAF6EF', border: '1px solid rgba(212,175,55,0.18)', boxShadow: '0 24px 64px rgba(26,26,26,0.14)' }}
              >
                <img src={imgSrc(item.image)} alt={item.headline} className="w-full h-full object-cover" loading="lazy" decoding="async" />
              </div>

              {item.pdf && (
                <motion.a
                  href={item.pdf}
                  target="_blank"
                  rel="noopener noreferrer"
                  whileHover={{ scale: 1.04 }}
                  whileTap={{ scale: 0.97 }}
                  className="mt-6 flex items-center justify-center gap-2 font-ui text-[0.62rem] tracking-[0.18em] uppercase px-4 py-2.5 transition-colors duration-200"
                  style={{ color: 'var(--gold-dark)', border: '1px solid rgba(212,175,55,0.3)' }}
                  onMouseEnter={e => (e.currentTarget.style.color = 'var(--gold)')}
                  onMouseLeave={e => (e.currentTarget.style.color = 'var(--gold-dark)')}
                >
                  <FileText size={12} /> View Full Feature (PDF)
                </motion.a>
              )}
            </motion.div>

            {/* Content */}
            <motion.div
              initial={{ opacity: 0, x: 24 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6, delay: 0.1, ease: [0.25, 0.46, 0.45, 0.94] }}
            >
              <motion.button
                initial={{ opacity: 0, x: -12 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.4 }}
                onClick={() => router.push('/media/articles')}
                className="flex items-center gap-2 font-ui text-xs tracking-widest uppercase mb-8 transition-colors duration-300"
                style={{ color: 'var(--gold-dark)' }}
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--gold)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--gold-dark)')}
              >
                <ArrowLeft size={14} /> Back to Articles
              </motion.button>

              <div className="flex flex-wrap items-center gap-3 mb-5">
                <span
                  className="font-ui text-[0.5rem] tracking-[0.22em] uppercase px-2.5 py-1"
                  style={{ background: `${catColor}15`, color: catColor, border: `1px solid ${catColor}30` }}
                >
                  {item.category}
                </span>
                <span className="flex items-center gap-1.5 font-ui text-[0.5rem] tracking-widest uppercase" style={{ color: 'rgba(26,26,26,0.5)' }}>
                  <Calendar size={10} /> {item.date}
                </span>
              </div>

              <h1
                className="font-times font-normal leading-tight mb-6"
                style={{ fontSize: 'clamp(1.6rem, 3vw, 2.3rem)', color: 'var(--luxury-dark)', letterSpacing: '0.005em' }}
              >
                {item.headline}
              </h1>

              <div className="gold-line mb-6" />

              <div
                className="font-body leading-relaxed"
                style={{ color: 'rgba(26,26,26,0.85)', fontSize: '0.98rem', lineHeight: 1.9 }}
              >
                <p className="mb-5">
                  <strong style={{ color: 'var(--luxury-dark)' }}>{item.publication}, {item.date}: </strong>
                  {item.excerpt}
                </p>
                <p>
                  This piece is part of <strong style={{ color: 'var(--luxury-dark)' }}>{item.type}</strong>,
                  {' '}one of the brand campaigns and features Majestique Landmarks has shared with Pune across
                  the city's leading publications.
                </p>
              </div>

              <div
                className="flex items-center mt-12 pt-8"
                style={{ borderTop: '1px solid rgba(212,175,55,0.2)' }}
              >
                <Link
                  href="/media/articles"
                  className="flex items-center gap-2 font-ui text-xs tracking-widest uppercase transition-colors duration-300"
                  style={{ color: 'var(--gold-dark)' }}
                  onMouseEnter={e => (e.currentTarget.style.color = 'var(--gold)')}
                  onMouseLeave={e => (e.currentTarget.style.color = 'var(--gold-dark)')}
                >
                  <ArrowLeft size={14} /> All Articles
                </Link>
              </div>
            </motion.div>
          </div>
        </div>
      </div>

      {/* Related Articles */}
      {related.length > 0 && (
        <section className="py-14" style={{ background: 'var(--cream)', borderTop: '1px solid rgba(212,175,55,0.15)' }}>
          <div className="container-luxury">
            <p className="font-ui text-[0.5rem] tracking-[0.3em] uppercase text-center mb-10" style={{ color: 'rgba(212,175,55,0.6)' }}>
              More Articles
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
              {related.map((r, i) => {
                const rCatColor = ARTICLE_CATEGORY_META[r.category]?.color || '#D4AF37'
                return (
                  <motion.div
                    key={r.id}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.5, delay: i * 0.1 }}
                  >
                    <Link
                      href={`/media/articles/${r.slug}`}
                      className="group flex flex-col overflow-hidden bg-white h-full"
                      style={{ border: '1px solid rgba(212,175,55,0.16)', boxShadow: '0 2px 16px rgba(26,26,26,0.05)', transition: 'box-shadow 0.35s ease, transform 0.35s ease' }}
                      onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 18px 48px rgba(26,26,26,0.12)'; e.currentTarget.style.transform = 'translateY(-4px)' }}
                      onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 2px 16px rgba(26,26,26,0.05)'; e.currentTarget.style.transform = 'translateY(0)' }}
                    >
                      <div className="relative overflow-hidden" style={{ aspectRatio: '3/4', background: '#FAF6EF' }}>
                        <Image src={r.image} alt={r.headline} className="object-cover transition-transform duration-600 group-hover:scale-[1.05]" fill sizes="100vw" />
                        <div className="absolute top-2.5 left-2.5">
                          <span
                            className="font-ui text-[0.42rem] tracking-[0.2em] uppercase px-2 py-0.5"
                            style={{ background: rCatColor, color: '#fff' }}
                          >
                            {r.category}
                          </span>
                        </div>
                      </div>
                      <div className="p-4">
                        <span
                          className="font-ui text-[0.42rem] tracking-[0.2em] uppercase mb-2 inline-block px-2 py-0.5"
                          style={{ background: `${r.pubColor}14`, color: r.pubColor, border: `1px solid ${r.pubColor}28` }}
                        >
                          {r.publication}
                        </span>
                        <h3 className="font-times font-normal leading-snug" style={{ fontSize: '0.92rem', color: 'var(--luxury-dark)' }}>
                          {r.headline}
                        </h3>
                      </div>
                    </Link>
                  </motion.div>
                )
              })}
            </div>
          </div>
        </section>
      )}
    </>
  )
}
