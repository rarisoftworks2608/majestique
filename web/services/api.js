...
/* The base URL was previously hardcoded with a `|| 'http://localhost...'`
   fallback that could never fire (a non-empty string is always truthy).
   It is env-driven now so local, staging and production differ by config. */
export const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || 'https://real-estate-project-production-da1c.up.railway.app/api'
...
api.interceptors.request.use((config) => {
  /* Guarded for SSR: this module is imported by client components, but the
     interceptor must not assume `localStorage` exists during prerender. */
