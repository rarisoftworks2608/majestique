...
function GoogleWordmark() {
  return (
    <div className="flex flex-wrap items-center gap-1.5 w-full">
      <svg viewBox="0 0 48 48" style={{ width: '16px', height: '16px', flexShrink: 0 }} aria-hidden="true">
      ...
      <span className="leading-none" style={{ fontFamily: '"Product Sans", Arial, sans-serif', fontSize: '0.92rem', letterSpacing: '-0.01em' }}>
      ...
      <span className="font-ui leading-none" style={{ fontSize: '0.74rem', color: 'rgba(26,26,26,0.72)', fontWeight: 500 }}>
        Reviews
      </span>
    </div>
  )
...
function StarRating({ value }) {
  ...
  /* role="img" is required for aria-label to be permitted on a generic
     container — without it the label is ignored and axe flags it. */
  return (
    <div className="flex items-center gap-1" role="img" aria-label={`${value} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        if (i === totalFull && hasHalf) {
          return (
            <span key={i} className="relative inline-flex" style={{ width: 15, height: 15 }}>
              <Star size={15} color="rgba(212,175,55,0.3)" style={{ position: 'absolute', inset: 0 }} />
        ...
        return <Star key={i} size={15} color="rgba(212,175,55,0.3)" />
      })}
    </div>
  )
}
...
export default function GoogleReviewsBadge({ className = '' }) {
  return (
    <div
      className={`relative flex flex-col items-start gap-1.5 box-border w-full max-w-[210px] ${className}`}
      style={{ background: '#FAF6EF', border: '1px solid rgba(212,175,55,0.22)', boxShadow: '0 14px 36px rgba(0,0,0,0.3)', padding: '0.85rem 1rem' }}
    >
      ...
      <GoogleWordmark />
      <p className="font-body text-xs" style={{ color: 'rgba(26,26,26,0.72)' }}>{SITE_NAME}</p>
