...
function ensureScript() {
  ...
  /* Explicit https rather than a protocol-relative URL, which resolved to
     plain http and tripped the mixed-content / "does not use HTTPS" audit. */
...
/* Used as a useState initializer, which also runs during server render —
   fall back to English when there is no document to read. The real value
   is picked up on the client in an effect below. */
...
export default function GoogleTranslate({ className = '' }) {
  ...
  /* The server render can't read the googtrans cookie, so it always emits
     English. Re-read it after hydration to show the real active language. */
  useEffect(() => {
      ...
      <div ref={ref} className={`relative hidden lg:block notranslate ${className}`}>
        <button
          ...
          /* WCAG 2.5.3 (Label in Name): the accessible name must contain the
             visible text, which here is the language code. */
          aria-label={`${selected.displayCode}, select language`}
          aria-expanded={open}
...
export function MobileLanguageSelector({ onAfterSelect }) {
  ...
  /* See the desktop selector above — cookie is client-only. */
  useEffect(() => {
    setSelected(getLangFromCookie())
  }, [])
