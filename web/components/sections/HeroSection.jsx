...
export default function HeroSection() {
  ...
  /* ── Init YouTube player ──
     Deferred until the browser is idle. The player pulls ~6.4MB of video
     plus ~900KB of YouTube JS; starting it during initial load made it
     compete with LCP. The poster image below covers the hero until the
     video is ready, so the visual result is unchanged. */
  useEffect(() => {
    let cancelled = false
    let idleHandle = null
  ...
  return (
    ...
    >
      {/* ── Poster frame ──
          The hero's LCP element. Served from our own origin as an optimized
          AVIF/WebP and preloaded, so the hero paints immediately instead of
          waiting on the YouTube player. It stays underneath the video and
          fades out once playback actually starts. */}
      <div
        className="absolute inset-0 overflow-hidden pointer-events-none transition-opacity duration-1000"
        style={{ opacity: playerReady && playing ? 0 : 1 }}
        aria-hidden="true"
      >
        <Image
          src={heroPoster}
