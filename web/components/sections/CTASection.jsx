...
export default function CTASection() {
  ...
  /* Scroll-scrubbed parallax, previously GSAP ScrollTrigger. Framer Motion
     is already in the bundle, so this drops gsap + ScrollTrigger entirely. */
  const { scrollYProgress } = useScroll({ target: sectionRef, offset: ['start end', 'end start'] })
  const y = useTransform(scrollYProgress, [0, 1], ['-6%', '6%'])

  return (
    <section ref={sectionRef} className="relative overflow-hidden" style={{ background: '#FAF6EF' }}>
      ...
      <div className="absolute top-0 left-0 w-72 h-72 opacity-[0.035] pointer-events-none"
        style={{ backgroundImage: 'radial-gradient(circle,rgba(212,175,55,1) 1px,transparent 1px)', backgroundSize: '22px 22px' }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[55%_45%]" style={{ minHeight: '80vh' }}>
        ...
        <div className="flex flex-col justify-center py-16 lg:py-24 px-8 lg:px-16 relative z-10">
          ...
          <motion.p
            ...
            style={{ fontSize: 'clamp(1rem, 1.3vw, 1.125rem)', color: 'rgba(26,26,26,0.84)', fontWeight: 400, maxWidth: '440px', lineHeight: 1.85 }}
          >
          ...
          >
            <Link href="/contact"
              ...
              style={{ fontSize: '0.6rem', padding: '1rem 2.4rem', background: '#6B0D1A', color: '#fff', border: '1px solid #6B0D1A' }}
            ...
            <a href="tel:+917448099000"
              className="inline-flex items-center gap-2.5 font-ui tracking-[0.2em] uppercase transition-all duration-300"
              style={{ fontSize: '0.6rem', padding: '1rem 2.4rem', background: 'transparent', color: 'var(--ink)', border: '1px solid rgba(26,26,26,0.28)' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--gold)'; e.currentTarget.style.color = 'var(--gold-dark)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(26,26,26,0.28)'; e.currentTarget.style.color = 'var(--ink)' }}
            >
            ...
            <Link href="/projects"
              ...
              style={{ fontSize: '0.6rem', color: 'rgba(26,26,26,0.38)' }}
          ...
          <motion.div
            ...
            style={{ borderTop: '1px solid rgba(212,175,55,0.15)', paddingTop: '1.75rem' }}
          >
            {TRUST.map(badge => (
              <div key={badge} className="flex items-center gap-2">
                <div className="w-1 h-1 rounded-full" style={{ background: 'var(--gold)' }} />
                <span className="font-ui text-[0.46rem] tracking-[0.18em] uppercase" style={{ color: 'rgba(26,26,26,0.38)' }}>
                  {badge}
                </span>
              </div>
            ))}
        ...
        <div className="relative overflow-hidden hidden lg:block">
          <div className="absolute inset-0 overflow-hidden">
            <motion.div
              style={{ y, willChange: 'transform', transform: 'scale(1.12)' }}
              className="absolute inset-0"
            >
              <Image
                src={imgVenice}
                alt="Majestique Venice residential tower elevation, Pune"
                fill
                sizes="45vw"
          ...
          <div className="absolute inset-y-0 left-0 w-20"
            style={{ background: 'linear-gradient(to right, #FAF6EF, transparent)' }} />

          {/* Subtle overlay to keep premium but bright */}
          <div className="absolute inset-0"
            style={{ background: 'linear-gradient(to bottom, rgba(250,246,239,0.08) 0%, rgba(250,246,239,0.12) 100%)' }} />
          ...
          >
            ...
            <p className="font-ui text-[0.46rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(26,26,26,0.42)' }}>
